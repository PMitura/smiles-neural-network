...
def configureModel(input):
    ...
    model.add(TimeDistributed(Dense(500, activation = 'tanh', trainable = RP['trainable_inner']), input_shape = (None, alphaSize )))
    ...
    model.add(Dropout(0.5))
    model.add(GRU(500, trainable = RP['trainable_inner']))
    model.add(Activation('relu', trainable = RP['trainable_inner']))
    model.add(Dropout(0.5))
    # model.add(Dense(200) )
    # model.add(Activation('relu', trainable = RP['trainable_inner']))
