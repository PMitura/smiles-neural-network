...
# suppl = SDMolSupplier('../local/data/test.sdf')
...
while total < len(smiles):

    lehi = min(total+LIMIT,len(smiles))
    rows = lehi - total

    df = pd.read_csv('../local/data/output.target_molweight_features_wide.3d_desc.csv',skiprows=idx,nrows=rows,
    ...
    df['canonical_smiles'] = pd.Series(chunk,index=df.index)
